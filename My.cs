...
namespace My
{
  ...
  // MyFunctioNs
  {
    public static void GetRatings(string line, string cookie)
    {
      try
      {
        String str = "";
        WebClient wcu = new WebClient();
        wcu.Headers.Add("Cookie: " + cookie);
        wcu.Headers.Add("Content-Type", "application/x-www-form-urlencoded");
        str = wcu.DownloadString(new Uri(@"http://www.imdb.com/list/export?list_id=ratings&author_id=ur" + line));
        string[] sar = str.Replace("'", "''").Split('\n');// (new string[] { Environment.NewLine }, StringSplitOptions.None);
        string cmdstr = "";
        int dolusatir = 0;
        for (int i = 1; i < sar.Length; i++) // 
          if (sar[i].Trim() != "")
          {
            cmdstr += "if not exists(Select top 1 1 from Rating r where r.voterid='" + line + "' and r.movieid='" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[1].Substring(2) + "') " +
              "insert into Rating (voterid, movieid, MovieName, MovieType, Directors, Rate, IMDBRate, Runtime, Year, Genres, VoteCount) " +
              " Select '" + line + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[1].Substring(2) + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[5] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[6] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[7] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[8] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[9] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[10] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[11] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[12] + "', '" + sar[i].Split(new string[] { "\",\"" }, StringSplitOptions.None)[13] + "'" + Environment.NewLine;
            dolusatir++;
          }
        mfn.GetSqlResult(cmdstr);
        //mfn.GetSqlResult("Update Voter Set RateCount=" + dolusatir.ToString() + " where voterid='" + line + "'");
        mfn.GetSqlResult("Exec sp_PrepareTables '" + line + "'");
        //
      }
      catch (Exception ex)
      {
    ...
    public static SqlConnection GetConnection(string ConnectionName = "")
    {
      ...
      {
        //if (HttpContext.Current.Server.MachineName=="BILGISAYAR") // 70.35.206.36,1002 // MUHASEBESRV\\MUH2014
        //        connstr = "Server=185.48.213.235,1236; Database=master; uid=sa; pwd=dgfceu;pooling=true; connection lifetime=10; connection timeout=0; packet size=1024;";
        //        connstr = "Server=.; Database=MovieRate; uid=sa; pwd=e;pooling=true; connection lifetime=10; connection timeout=0; packet size=1024;";
        //        connstr = "Server=70.35.206.36,1002; Database=MovieRate; uid=sa; pwd=Erci1572;pooling=true; connection lifetime=10; connection timeout=0; packet size=1024;";
        connstr = "Server=.; Database=MovieRate; uid=sa; pwd=Erci1572;pooling=true; connection lifetime=10; connection timeout=0; packet size=1024;";
      }
